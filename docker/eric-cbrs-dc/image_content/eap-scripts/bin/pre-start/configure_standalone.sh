...
updateDefaultProperties() {
    ...
    if [[ ! -z "$CONNECT_MODE" ]]; then
       info "Inserting CONNECT_MODE=$CONNECT_MODE into JBoss default.properties"
       echo "CONNECT_MODE=${CONNECT_MODE}" >> $DEFAULT
    fi

    info "Inserting DISABLE_SYSLOG=true into JBoss default.properties"
    echo "DISABLE_SYSLOG=true" >> $DEFAULT
}
