    ...
    stages {
		...
		options {
				...
				stage('Nmap') {
						steps {
						  script {
                                 ...
                                 for (service in val) {
                                      env.SERVICE = service
                                      sh "cat ./nmap/nmap_template.yaml |sed -e 's/SERVICE_NAME/${SERVICE}/'>>${FILENAME}"
                                      sh "echo ''>>${FILENAME}"
                                  }
		   ...
		   stage('CIS CAT') {
		               agent{
		                node {
			                  label "Jenkins_FEM9s11_podh_CIAgent_6"
		                     }
							 }
						steps {
						  sh "${bob} init-drop"
						  sh "${bob} cis-cat-assessor-scan"
						  stash includes: 'build/cis-cat_reports/*.*', name: 'ciscat_reports'
						 archiveArtifacts 'cis_cat_assessor/cis-cat_reports/**.*'
						}
					}
				stage('Generate Vulnerability report V2.0'){
                            steps {
							    unstash  name: 'ciscat_reports'
                                sh "${bob} generate-full-VA-report-V2:no-upload"
                            }
                        }
            }
        }
